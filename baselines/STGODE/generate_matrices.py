...
def generate_dtw_spa_matrix(dataset_name, sigma1=0.1, thres1=0.6, sigma2=10, thres2=0.5):
    ...
    if dataset_name in ["DYG_data_3_sub1-3", "DYG_data_3_sub2-3", "DYG_data_3_sub3"]:
        print(f"STGODE generate spatial matrix based on the raw data. Please ensure the raw data is placed in the correct path `datasets/raw_data/{dataset_name}/sub1_casualgraph.csv.")
        if not os.path.exists('{0}/{1}_spatial_distance.npy'.format(os.path.abspath(__file__ + "/.."), dataset_name)):
            graph_csv_file_path = "./datasets/raw_data/{0}/sub1_casualgraph.csv".format(dataset_name)
            # 初始化距离矩阵
        ...
        # normalization
        # 打印 dist_matrix 查看其内容
        print("Distance matrix before normalization:", dist_matrix)
        ...
        if std == 0:
            ...
            # 如果 std 为 0，可以选择直接减去均值，而不做归一化
            dist_matrix = dist_matrix - mean
        else:
            dist_matrix = (dist_matrix - mean) / std
        ...
        sp_matrix[sp_matrix < thres2] = 0  # 应用阈值
    else:
...
if __name__ == "__main__":
    ...
    generate_dtw_spa_matrix("DYG_data_3_sub1-3")
